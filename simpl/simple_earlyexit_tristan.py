...
class FusionNet(nn.Module):
    def __init__(self, device, config):
        ...
        self.exit_head = ExitHead(
            hidden_size=d_embed,
            num_modes=config['g_num_modes'],
            future_steps=config['g_pred_len']
        )

        self.exit_layer = 2 #need to test with different exit layers
    ...
    def forward(self, actors, actor_idcs, lanes, lane_idcs, rpe_prep):

        actors = self.proj_actor(actors)
        lanes = self.proj_lane(lanes)

        # Each entry is either:
        #   ('exit', exit_cls, exit_traj)          -- scene took early exit
        #   ('full', actor_tokens, lane_tokens)     -- scene ran all layers
        scene_results = []

        # -------- PER SCENE --------
        for scene_idx in range(len(actor_idcs)):

            a_idcs = actor_idcs[scene_idx]
            l_idcs = lane_idcs[scene_idx]
            rpes = rpe_prep[scene_idx]
            ...
            edge_mask = rpes['scene_mask']
            edge = rpe

            exited_this_scene = False

            # -------- LAYERS --------
            for layer_idx, layer in enumerate(self.fuse_scene.fusion):

                tokens, edge, _ = layer(tokens, edge, edge_mask)

                if layer_idx == self.exit_layer:
                    actor_tokens = tokens[:len(a_idcs)]

                    exit_cls, exit_traj = self.exit_head(actor_tokens)

                    confidence = exit_cls.max(dim=1)[0].mean()

                    if confidence > self.exit_threshold:
                        exited_this_scene = True
                        scene_results.append(('exit', exit_cls, exit_traj))
                        break

            if not exited_this_scene:
                # Finished all layers — store final tokens for pred_net
                scene_results.append(('full', tokens[:len(a_idcs)], tokens[len(a_idcs):]))

        # -------- COLLECT RESULTS --------
        # Check if any scene needs pred_net
        full_indices = [i for i, r in enumerate(scene_results) if r[0] == 'full']

        if not full_indices:
            # All scenes exited early — build output lists directly
            batch_res_cls, batch_res_reg, batch_res_aux = [], [], []
            for _, exit_cls, exit_traj in scene_results:
                dummy_vel = torch.zeros_like(exit_traj)
                batch_res_cls.append(exit_cls)
                batch_res_reg.append(exit_traj)
                batch_res_aux.append((dummy_vel, None))
            return batch_res_cls, batch_res_reg, batch_res_aux, True

        # At least one scene needs pred_net — run it for those scenes
        # Gather actor tokens and corresponding actor_idcs for full-pass scenes
        full_actor_tokens = torch.cat([scene_results[i][1] for i in full_indices], dim=0)
        full_actor_idcs = []
        offset = 0
        for i in full_indices:
            n = scene_results[i][1].shape[0]
            full_actor_idcs.append(torch.arange(offset, offset + n, device=full_actor_tokens.device))
            offset += n

        pred_cls, pred_reg, pred_aux = self.pred_net_forward(full_actor_tokens, full_actor_idcs)

        # Merge exit and full-pass results back in original scene order
        batch_res_cls, batch_res_reg, batch_res_aux = [], [], []
        full_iter = iter(zip(pred_cls, pred_reg, pred_aux))

        for result in scene_results:
            if result[0] == 'exit':
                _, exit_cls, exit_traj = result
                dummy_vel = torch.zeros_like(exit_traj)
                batch_res_cls.append(exit_cls)
                batch_res_reg.append(exit_traj)
                batch_res_aux.append((dummy_vel, None))
            else:
                p_cls, p_reg, p_aux = next(full_iter)
                batch_res_cls.append(p_cls)
                batch_res_reg.append(p_reg)
                batch_res_aux.append(p_aux)
...
class Simpl(nn.Module):
    ...
    def __init__(self, cfg, device):
        ...
        # Give fusion_net a reference to pred_net so it can run full-pass scenes
        self.fusion_net.pred_net_forward = self.pred_net.forward
    ...
    def forward(self, data):
        ...
        # fusion_net now always returns fully populated per-scene lists
        res_cls, res_reg, res_aux, _ = self.fusion_net(actors, actor_idcs, lanes, lane_idcs, rpe)

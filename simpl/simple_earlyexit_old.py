...
class FusionNet(nn.Module):
    ...
    def forward(self, actors, actor_idcs, lanes, lane_idcs, rpe_prep):
        ...
        for a_idcs, l_idcs, rpes in zip(actor_idcs, lane_idcs, rpe_prep):
            ...
            for layer in self.fuse_scene.fusion:
                tokens, edge, _ = layer(tokens, edge, edge_mask)

            actors_new.append(tokens[:len(a_idcs)])
            lanes_new.append(tokens[len(a_idcs):])

        actors = torch.cat(actors_new, dim=0)
        lanes = torch.cat(lanes_new, dim=0)

        return actors, lanes, None, False
...
class Simpl(nn.Module):
    ...
    def forward(self, data):
        ...
        fusion_out = self.fusion_net(actors, actor_idcs, lanes, lane_idcs, rpe)

        # ---- SAFE UNPACK ----
        if isinstance(fusion_out, tuple) and len(fusion_out) == 4:
            out1, out2, out3, early_exit_flag = fusion_out
        else:
            raise ValueError("Unexpected fusion output format")
